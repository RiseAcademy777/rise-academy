...
export default async function handler(req, res) {
  ...
  const { data: user } = await sb.from('users').select('id,name,role').eq('email', email).single();
  if (!user || !['teacher','director'].includes(user.role)) return res.status(403).json({ error: '강사 또는 원장 계정만 로그인 가능합니다.' });
