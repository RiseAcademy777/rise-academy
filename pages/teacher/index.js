...
export async function getServerSideProps(ctx) {
  ...
  if (classes && classes.length > 0) {
    ...
    const { data } = await sb.from('users').select('id,name,class_id,classes(id,name,level_code)').in('class_id', classIds).eq('role', 'student').order('name');
    students = data || [];
  }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
          <div className="grid grid-cols-4 gap-2">
            {menus.map(m => (
              <button key={m.href} onClick={() => router.push(m.href)} className="bg-white rounded-xl border border-gray-200 p-3 text-center hover:border-gray-300 transition-colors relative">
              ...
              return (
                <div key={student.id} className="bg-white rounded-xl border border-gray-200 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button onClick={() => generateLink(student.id)} disabled={generating[student.id]} className="text-xs px-3 py-1.5 rounded-lg bg-gray-900 text-white disabled:opacity-50 whitespace-nowrap hover:bg-gray-700 flex-shrink-0">
                  ...
                  {isValid && (
                    <>
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <button onClick={() => copyLink(student.id, token)} className={`text-xs px-3 py-2 rounded-lg border whitespace-nowrap flex-shrink-0 transition-colors ${copied[student.id] ? 'bg-green-50 border-green-200 text-green-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
