 ...
 const { data: classes } = await sb
  ...
  if (sIds.length > 0) {
    const { data: tokens } = await sb
      ...
      .select('student_id,token,expires_at,is_active,created_at')
      .in('student_id', sIds)
      .eq('is_active', true)
      .order('created_at', { ascending: false });
    (tokens || []).forEach(t => {
      if (!tokenMap[t.student_id]) tokenMap[t.student_id] = t;
    });
  }

  const { count: pendingNotices } = await sb
    .from('notices')
...
export default function TeacherDashboard({ teacher, students, tokenMap, pendingNotices, pendingReports, siteUrl }) {
  ...
  return (
    <>
      <Head>
        <title>강사 대시보드 · Rise Language Academy</title>
          ...
          <div className="grid grid-cols-4 gap-2">
            {menus.map(m => (
              <button key={m.href} onClick={() => router.push(m.href)}
                ...
                {m.badge > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-xs rounded-full flex items-center justify-center">{m.badge}</span>
                )}
