...
export async function getServerSideProps(ctx) {
  try {
    ...
    const { data: sess } = await sb.from('teacher_sessions').select('teacher_id,expires_at').eq('session_key', sessionKey).maybeSingle();
    if (!sess) return { redirect: { destination: '/login', permanent: false } };
    if (new Date(sess.expires_at) < new Date()) return { redirect: { destination: '/login', permanent: false } };

    const tid = sess.teacher_id;

    const { data: teacher } = await sb.from('users').select('id,name,role').eq('id', tid).maybeSingle();
    const { data: studentsRaw } = await sb.from('users').select('id,name,class_id,classes(id,name,level_code)').eq('role','student').order('name');
    ...
    console.error('TEACHER_PAGE_ERROR:', String(e));
    throw e;
  }
}
...
export default function TeacherPage({ teacher, students, tokenMap, siteUrl }) {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {teacher?.role === 'director' && <button onClick={() => router.push('/admin')} className="text-xs text-blue-600">관리자</button>}
        ...
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
          <div className="grid grid-cols-4 gap-2">
            {menus.map(m => (
              <button key={m.href} onClick={() => router.push(m.href)} className="bg-white rounded-xl border border-gray-200 p-3 text-center hover:border-gray-300">
          ...
          <p className="text-xs text-gray-400 text-center">전체 학생 {students.length}명</p>
          {students.length === 0 && <div className="text-center py-12 text-sm text-gray-400">등록된 학생이 없습니다.</div>}
              ...
              <div key={student.id} className="bg-white rounded-xl border border-gray-200 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center text-sm font-semibold text-blue-700 flex-shrink-0">{student.name.charAt(0)}</div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{student.name}</p>
                      <p className="text-xs text-gray-400">{student.classes?.name}</p>
                    </div>
                  </div>
                  <button onClick={() => generateLink(student.id)} disabled={gen[student.id]} className="text-xs px-3 py-1.5 rounded-lg bg-gray-900 text-white disabled:opacity-50 whitespace-nowrap flex-shrink-0">
                ...
                {isValid && (
                  <>
                    <div className="mt-3 flex items-center gap-2">
                      <div className="flex-1 min-w-0 text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2 font-mono truncate border border-gray-100">{siteUrl}/r/{token.token}</div>
                      <button onClick={() => copyLink(student.id, token)} className={`text-xs px-3 py-2 rounded-lg border whitespace-nowrap flex-shrink-0 ${copied[student.id] ? 'bg-green-50 border-green-200 text-green-700' : 'border-gray-200 text-gray-600'}`}>
